...
DMG_STAGING_DIR="$ROOT_DIR/.build/release/GlobalPetAssistant-dmg"
...
(cd "$(dirname "$ZIP_PATH")" && shasum -a 256 "$(basename "$ZIP_PATH")") > "$CHECKSUM_PATH"

mkdir -p "$DMG_STAGING_DIR"
ditto "$APP_PATH" "$DMG_STAGING_DIR/GlobalPetAssistant.app"
ln -s /Applications "$DMG_STAGING_DIR/Applications"
hdiutil create \
  -volname "Global Pet Assistant" \
  -srcfolder "$DMG_STAGING_DIR" \
  -format UDZO \
  -ov \
  "$DMG_PATH"
rm -rf "$DMG_STAGING_DIR"
(cd "$(dirname "$DMG_PATH")" && shasum -a 256 "$(basename "$DMG_PATH")") > "$DMG_CHECKSUM_PATH"
